...
<head>
    ...
    <title>SANAR + | Editar Tratamiento</title>
...
<div class="max-w-3xl mx-auto">
    ...
    <h1 class="text-3xl font-extrabold text-[#2c3e50] mb-8 tracking-wide">Editar Tratamiento</h1>
        ...
        <form action="{{ route('tratamientos.update', $tratamiento->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="diagnosis_id" class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#5db7a1] transition bg-white" required>
                        @foreach($diagnosticos as $diagnostico)
                            <option value="{{ $diagnostico->id }}" {{ old('diagnosis_id', $tratamiento->diagnosis_id) == $diagnostico->id ? 'selected' : '' }}>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="general_indications" rows="3"
                        class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#5db7a1] transition resize-none" required>{{ old('general_indications', $tratamiento->{'general indications'}) }}</textarea>
                </div>

                <div>
                    <label class="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Fecha de Inicio</label>
                    <input type="date" name="start_date" value="{{ old('start_date', $tratamiento->start_date) }}"
                ...
                <div>
                    ...
                    <input type="date" name="end_date" value="{{ old('end_date', $tratamiento->end_date) }}"
            ...
            <div class="flex items-center justify-end gap-3 mt-8 pt-6 border-t border-gray-100">
                ...
                <button type="submit" class="px-6 py-2.5 text-sm font-semibold text-white bg-[#1a73e8] hover:bg-[#1557b0] rounded-lg shadow-sm transition-colors">Actualizar Tratamiento</button>
            </div>
        </form>
    </div>
</div>
