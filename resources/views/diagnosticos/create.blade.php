...
<head>
    ...
    <title>SANAR + | Nuevo Diagnóstico</title>
...
<div class="max-w-3xl mx-auto">
    ...
    <h1 class="text-3xl font-extrabold text-[#2c3e50] mb-8 tracking-wide">Agregar Nuevo Diagnóstico</h1>
        ...
        <form action="{{ route('diagnosticos.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <select name="date_id" class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#5db7a1] transition bg-white" required>
                        ...
                        @foreach($citas as $cita)
                            <option value="{{ $cita->id }}" {{ old('date_id') == $cita->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="text" name="disease" value="{{ old('disease') }}" placeholder="Ej: Hipertensión arterial"
                ...
                <div>
                    ...
                    <textarea name="description_clinical" rows="4" placeholder="Describe los síntomas y hallazgos clínicos..."
                        class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#5db7a1] transition resize-none" required>{{ old('description_clinical') }}</textarea>
                </div>
            ...
            <div class="flex items-center justify-end gap-3 mt-8 pt-6 border-t border-gray-100">
                ...
                <button type="submit" class="px-6 py-2.5 text-sm font-semibold text-white bg-[#1a73e8] hover:bg-[#1557b0] rounded-lg shadow-sm transition-colors">Guardar Diagnóstico</button>
            </div>
        </form>
    </div>
</div>
