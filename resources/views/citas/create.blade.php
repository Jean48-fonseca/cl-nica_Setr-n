...
<div class="max-w-3xl mx-auto">
    ...
    <h1 class="text-3xl font-extrabold text-[#2c3e50] mb-8 tracking-wide">Agregar Nueva Cita</h1>
        ...
        <form action="{{ route('citas.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="doctor_id" class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#5db7a1] transition bg-white" required>
                        ...
                        @foreach($medicos as $medico)
                            <option value="{{ $medico->id }}" {{ old('doctor_id') == $medico->id ? 'selected' : '' }}>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="motivo" value="{{ old('motivo') }}" placeholder="Ej: Consulta general, dolor de cabeza..."
                        class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#5db7a1] transition" required>
                </div>
            ...
            <div class="flex items-center justify-end gap-3 mt-8 pt-6 border-t border-gray-100">
                ...
                <button type="submit" class="px-6 py-2.5 text-sm font-semibold text-white bg-[#1a73e8] hover:bg-[#1557b0] rounded-lg shadow-sm transition-colors">Guardar Cita</button>
            </div>
        </form>
    </div>
</div>
